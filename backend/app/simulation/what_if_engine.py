...
from app.ml.model_store import predict_recoverability, predict_recoverability_batch
...
def run_what_if_simulation(
    ...
    preloaded_failed_txns: Optional[List[Transaction]] = None,
    preloaded_gtw_map: Optional[Dict[int, Gateway]] = None,
    preloaded_p_ml_list: Optional[List[float]] = None,
    preloaded_baseline_eval: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
    """
    ...
    Optimized for ultra-fast batch ML inference and zero redundant DB queries.
    """
    gateway_status_overrides = gateway_status_overrides or {}
    strategy_overrides = strategy_overrides or {}

    if preloaded_failed_txns is not None:
        failed_txns = preloaded_failed_txns
    else:
    ...
    if transactions_considered == 0:
        return {
            ...
            "metrics": {
                ...
                "predicted_recoverable_amount": 0.0,
                "gross_expected_recovery": 0.0,
                "expected_recovery_cost": 0.0,
                "expected_friction": 0.0,
                "expected_risk_penalty": 0.0,
                "expected_net_recovery": 0.0,
                "expected_attempts": 0,
                "expected_successes": 0.0,
                "expected_success_rate": 0.0,
                "revenue_per_action": 0.0,
                "cost_per_successful_recovery": 0.0,
                "avoided_attempts": 0,
                "incremental_revenue_vs_baseline": 0.0,
                "incremental_net_recovery_vs_baseline": 0.0
            },
            "baseline_comparison": {}
        }

    # Prepare infer inputs for ML model batch inference
    infer_inputs = []
    ...
    # Batch ML inference if predictions not pre-supplied
    if preloaded_p_ml_list is not None:
        p_ml_list = preloaded_p_ml_list
    else:
        p_ml_list = predict_recoverability_batch(infer_inputs)
    ...
    for idx, t in enumerate(failed_txns):
        amount_dec = to_decimal(t.amount)
        infer_input = infer_inputs[idx]
        p_ml = p_ml_list[idx]
    ...
    if include_baseline:
        if preloaded_baseline_eval is not None:
            base_eval = preloaded_baseline_eval
        else:
